...
Output: A dictionary with key as a node id and value as the list/tuple of directions allowed for packet movement from that node
    ...
    if topology=='torus':
        ...
        for row_index in range (0, len(network)):
            for col_index in range (0, len(network[row_index])):
                available_directions[network[row_index][col_index]] = list_of_directions
